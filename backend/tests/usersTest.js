...
describe('Users', () => {
  ...
  describe('/POST user (register)', () => {
    it('it should POST new user (register)', (done) => {
      chai.request(server)
          .post('/api/register')
          .send(user)
          .end((err, res) => {
    ...
    describe('/GET user', () => {
        it('it should GET certain user', (done) => {
            chai.request(server)
                .post('/api/register')
                .send(user)
                .end((err, res) => {
                        ...
                        .end((err, res) => {
                              ...
                              try {
                                chai.request(server)
                                .get(`/api/${res.body[0]._id}`)
                                .end((err, res) => {
                                        res.should.have.status(200);
                                        res.body.should.be.a('object');
                                        res.body.username.should.be.eql("testName");
                                        res.body.email.should.be.eql("email@test.com");
                                    done();
                                });
                              } catch (err) {
                                  console.log(err)
    ...
    describe('/POST user (login)', () => {
        it('it should GET certain user (login)', (done) => {
            chai.request(server)
                .post('/api/register')
                .send(user)
                .end((err, res) => {
    ...
    describe('/PUT user (refresh Token)', () => {
        it('it should GET certain user (refresh Token)', (done) => {
            chai.request(server)
                .post('/api/register')
                .send(user)
                .end((err, res) => {
                        ...
                        .end((err, res) => {
                              ...
                              try {
                                chai.request(server)
                                    .put(`/api/refreshToken/${res.body[0]._id}`)
                                    .end((err, res) => {
                                            res.should.have.status(200);
                                            res.body.should.be.a('string');
                                            assert.notEqual(res.body[0].token, oldToken);
                                            done();
                                    });
                              } catch (err) {
                                  console.log(err)
    ...
    describe('/PUT user (edit)', () => {
        it('it should PUT certain user', (done) => {
            chai.request(server)
                .post('/api/register')
                .send(user)
                .end((err, res) => {
                        ...
                        .end((err, res) => {
                              ...
                              try {
                                ...
                                .end((err, res) => {
                                    res.should.have.status(200);
                                    res.body.should.be.a('string');
                                    res.body.should.be.eql("Account has been updated");
                                    chai.request(server)
                                        .get(`/api/${userId}`)
                                        .end((err, res) => {
                                                res.should.have.status(200);
                                                res.body.should.be.a('object');
                                                res.body.desc.should.be.eql("abc");
                                            done();
                                        });
                                });
                              } catch (err) {
    ...
    describe('/POST upload picture', () => {
        it('it should POST upload new picture', (done) => {
            chai.request(server)
                .post('/api/register')
                .send(user)
                .end((err, res) => {
                        ...
                        .end((err, res) => {
                              ...
                              try {
                                ...
                                .send({userId: res.body[0]._id, profilePicture: base64Image})
                                .end((err, res) => {
